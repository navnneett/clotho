...
export default function QuestionOne() {
    ...
    const [quizCompleted, setQuizCompleted] = useState(false); // Flag to track quiz completion

    const questionToAnswer = (answer) => {
        setSelectedAnswers([...selectedAnswers, answer]); // Add the selected answer to the array
    }
    ...
    const handleNextQuestion = () => {
        if (currentQuestion < 5) {
            ...
            setQuizCompleted(true); // Set the quiz completion flag to true
        }
    }

    // Function to calculate the final result based on selected answers
    ...
    return (
        <>
            ...
            {quizCompleted && <Result result={calculateResult()} />}
        </>
    );
}