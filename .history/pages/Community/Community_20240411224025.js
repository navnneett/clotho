...
export default function Community() {
    ...
    return (
        <>
            ...
            {
                data && Array.isArray(data.articles) && data.articles.slice(2, 3).map((d, index) => {
                    return(
                        <div key={index}>
                            <h3 style={{ color: 'var(--button-highlight-light)', fontSize: 'var(--open-sans-medium)', fontWeight: 'bold' }}>{d.title}</h3>
                            <p style={{ fontSize: 'var(--open-sans-small)', fontWeight: 'semibold' }}>
                                {d.description}
                                {d.author}
                            </p>
                        </div>
