...
export default function ThriftStore() {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            console.log("ZIP codes response:", response.data);
            ...
            if (response.data && response.data.DataList && response.data.DataList.length > 0) {
                ...
                const zippopotamResponses = await Promise.all(zipCodes.map(zipCode => axios.get(`https://api.zippopotam.us/CA/${zipCode}`)));
                console.log("Address details responses:", zippopotamResponses);
                
                const formattedResults = zippopotamResponses.map(response => response.data.places.map(place => ({
            ...
            setError('An error occurred while fetching the address. Please try again.');
        } finally {
            setIsLoading(false);
